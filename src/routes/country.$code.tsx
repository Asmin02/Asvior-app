...
function CountryHubPage() {
  ...
  const toggleFav = async () => {
    if (!userId) { toast.error("Sign in to save countries"); return; }
    if (isFav) {
    ...
    <div className="relative overflow-hidden pb-6">
      ...
      <section className="relative h-[340px]">
        ...
        <div className="absolute inset-x-0 bottom-0 px-6 pb-2 animate-fade-up">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            {visa && (
              <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider shadow-soft ${statusTone[visa.status]}`}>
          ...
          <p className="mt-2 max-w-sm text-sm leading-relaxed text-foreground/80">{profile.intro}</p>
        </div>
      </section>
      ...
      <section className="relative mt-4 px-6 animate-fade-up">
        <div className="glass rounded-3xl p-4">
          ...
          <CountryCombobox
            value={passport}
            onChange={(v) => { setPassport(v); savePassport(v); }}
          ...
          <div className="glass overflow-hidden rounded-3xl">
            ...
            <div className="space-y-3 p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <InfoTile icon={<Clock className="h-4 w-4" />} label="Max stay" value={visa.maxStay} />
                <InfoTile icon={<CalendarDays className="h-4 w-4" />} label="Processing" value={visa.processingTime} />
              </div>
              <InfoTile icon={<BadgeDollarSign className="h-4 w-4" />} label="Government fee" value={FEE_HINT[visa.status]} />
              <div className="grid grid-cols-1 gap-2 pt-1">
                ...
                >
                  <Globe2 className="h-4 w-4" /> Official visa website <ExternalLink className="h-3.5 w-3.5" />
                </a>
                <a
                  href={immigrationUrl}
                  target="_blank"
      ...
      <Section title="Travel cost" icon={<Banknote className="h-4 w-4" />}>
        <div className="glass rounded-3xl p-5">
          <div className="flex items-center justify-between">
            <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Trip length</p>
              ...
              {[3, 7, 14].map((d) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-[11px] font-bold transition-all active:scale-95 ${
                    days === d ? "gradient-primary text-primary-foreground shadow-soft" : "bg-muted text-muted-foreground"
                  }`}
                >
                  {d}d
                </button>
          ...
          <div className="mt-4 space-y-2.5">
            <CostRow emoji="🎒" label="Budget" daily={profile.cost.budget} days={days} tone="bg-emerald/12 text-emerald" />
            <CostRow emoji="🧳" label="Standard" daily={profile.cost.standard} days={days} tone="bg-primary/10 text-primary" />
            <CostRow emoji="👑" label="Luxury" daily={profile.cost.luxury} days={days} tone="bg-amber-400/15 text-amber-600 dark:text-amber-400" />
          </div>
      ...
      <Section title="Travel information" icon={<Compass className="h-4 w-4" />}>
        <div className="grid grid-cols-2 gap-3">
          <InfoCard icon={<CalendarDays className="h-4 w-4" />} label="Best season" value={profile.bestSeason} />
          <InfoCard icon={<Banknote className="h-4 w-4" />} label="Currency" value={profile.currency} />
          <InfoCard icon={<Languages className="h-4 w-4" />} label="Language" value={profile.language} />
          ...
          <InfoCard icon={<Phone className="h-4 w-4" />} label="Emergency" value={profile.emergency} />
        </div>
      </Section>
      ...
      <Section title="Local tips" icon={<Sparkles className="h-4 w-4" />}>
        <div className="space-y-2.5">
          <TipRow icon={<Utensils className="h-4 w-4" />} tone="bg-amber-400/15 text-amber-600 dark:text-amber-400" label="Food" text={profile.tips.food} />
          <TipRow icon={<Landmark className="h-4 w-4" />} tone="bg-primary/10 text-primary" label="Culture" text={profile.tips.culture} />
          <TipRow icon={<Bus className="h-4 w-4" />} tone="bg-emerald/12 text-emerald" label="Transport" text={profile.tips.transport} />
          <TipRow icon={<ShieldCheck className="h-4 w-4" />} tone="bg-destructive/10 text-destructive" label="Safety" text={profile.tips.safety} />
        </div>
      </Section>
        ...
        <div className="overflow-hidden rounded-3xl gradient-navy p-5 text-white shadow-float">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[11px] text-white/70">Instant answers from your travel concierge</p>
            </div>
          </div>
...
function CostRow({ emoji, label, daily, days, tone }: { emoji: string; label: string; daily: number; days: number; tone: string }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-muted/60 p-3">
      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-lg ${tone}`}>{emoji}</div>
...
function TipRow({ icon, tone, label, text }: { icon: React.ReactNode; tone: string; label: string; text: string }) {
  return (
    <div className="glass flex items-start gap-3 rounded-3xl p-4">
      <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl ${tone}`}>{icon}</div>
...
function DocChecklist({ passport, code, documents }: { passport: string; code: string; documents: string[] }) {
  ...
  return (
    <div className="glass rounded-3xl p-5">
      <div className="flex items-center justify-between">
        <p className="text-sm font-bold text-foreground">{done}/{documents.length} ready</p>
        <span className="rounded-full bg-emerald/12 px-2.5 py-0.5 text-[11px] font-bold text-emerald">{pct}%</span>
      </div>
      <div className="mt-2.5 h-2 overflow-hidden rounded-full bg-muted">
        <div className="h-full rounded-full gradient-emerald transition-all duration-500" style={{ width: `${pct}%` }} />
      </div>
      ...
      <p className="mt-3 text-center text-[10px] text-muted-foreground">Progress saves automatically on this device.</p>
    </div>
  );
}
