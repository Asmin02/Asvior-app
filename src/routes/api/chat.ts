      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          if (!Array.isArray(messages)) {
            console.error("[api/chat] Invalid payload", { traceId, body });
            return new Response("Messages are required", { status: 400 });
          }

          const key = process.env.LOVABLE_API_KEY;
          if (!key) {
            console.error("[api/chat] Missing LOVABLE_API_KEY", { traceId });
            return new Response("Missing LOVABLE_API_KEY", { status: 500 });
          }

          const model = process.env.ASVIOR_AI_MODEL || "google/gemini-3-flash-preview";
          ...
          console.error("[api/chat] Unhandled error", { traceId, err });
