...
function FavoritesPage() {
  ...
  const load = async () => {
    setLoadError(false);
    const { data, error } = await supabase.from("favorite_destinations").select("country_code").order("created_at", { ascending: false });
  ...
  const remove = async (code: string) => {
    const { error } = await supabase.from("favorite_destinations").delete().eq("country_code", code);
    if (error) { toast.error("Couldn't remove — check your connection and try again."); return; }
    load();
  };
          ...
          {favs.map((code) => (
            <Card key={code} className="ring-1 ring-border animate-fade-in">
              <CardContent className="p-4 text-center">
                ...
                <button onClick={() => remove(code)} className="mt-2 text-[11px] text-destructive hover:underline" aria-label={`Remove ${getCountryName(code)} from favorites`}>Remove</button>
              </CardContent>
            </Card>
          ))}
        </div>
