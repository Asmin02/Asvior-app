...
function ProfilePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      let resolved = data as Profile | null;

      if (!resolved) {
        const fullNameMeta = (u.user.user_metadata as { full_name?: string; name?: string } | undefined)?.full_name
          || (u.user.user_metadata as { full_name?: string; name?: string } | undefined)?.name
          || null;
        ...
        const { error: createErr } = await supabase.from("profiles").upsert(bootstrapProfile, { onConflict: "id" });
        if (createErr) throw createErr;
        resolved = bootstrapProfile;
      }

      setProfile(resolved);
      if (resolved.photo_url) {
        const { data: signed } = await supabase.storage.from("avatars").createSignedUrl(resolved.photo_url, 3600);
